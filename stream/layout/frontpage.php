...
// Carousel content.
// Sets a default hero image if none exists.
$defaultimage = $OUTPUT->image_url('default/homepagehero', 'theme');

// Fixed number of slides (3 images).
$slidestotal = 3;
$i = 0;
// Check for single carousel item to avoid unneeded html elements.
$slidestotal <= 1 ? $data['slides'][$i]['single'] = 1 : $data['slides'][$i]['single'] = 0;
// Define the three images from the pix folder.
$images = [
    $OUTPUT->image_url('default/1234', 'theme'),
    $OUTPUT->image_url('default/hero',  'theme'),
    $OUTPUT->image_url('default/hero1',  'theme')
];

// Cycle through the slider elements.
while ($i < $slidestotal) {
    // Dynamic name fields for text content.
    $title = "herotitle{$i}";
    $motto = "heromotto{$i}";
    $link = "herolink{$i}";
    $sliderbutton = "sliderbutton{$i}";
    // Matching mustache data and info.
    $data['slides'][$i]['herotitle'] = format_string($theme->settings->$title ?? 'Slide ' . ($i + 1));
    $data['slides'][$i]['slidermotto'] = format_string($theme->settings->$motto ?? '');
    $data['slides'][$i]['sliderlink'] = $theme->settings->$link ?? '';
    $data['slides'][$i]['sliderbutton'] = format_string($theme->settings->$sliderbutton ?? 'Learn More');
    $data['slides'][$i]['active'] = $i === 0; // Defining the starting point.
    $data['slides'][$i]['index'] = $i; // To trace pace.
    // Set image from the pix folder.
    $data['slides'][$i]['sliderimage'] = !empty($images[$i]) ? $images[$i] : $defaultimage;
    $i++;
}
...
// === CUSTOM: вывод реальных курсов Moodle ===
global $DB, $CFG;

// Получаем все видимые курсы (кроме фронтпейджа id=1)
...
$fs = get_file_storage(); // Moodle API для файлов

foreach ($courses as $course) {
    $context = context_course::instance($course->id);
    $courseimage = '';

    // Получаем изображение курса, если есть
    ...
    if (!empty($files)) {
        ...
        // Если картинки нет — используем дефолтную
...
// Добавляем список курсов в контекст шаблона
$templatecontext['courses'] = $courselist;

// Load JavaScript for carousel auto-slide.
$PAGE->requires->js_call_amd('theme_stream_child/carousel', 'init');
